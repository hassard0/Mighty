...
//! `sdust_diagnostics::codes` (the central catalog). This module
//! re-exports the same numeric values as bare `u16`s so that the
//! historical call-sites (`DiagCode::new(sdust_macros::diag::FOO)`,
//! `sdust_macros::UNKNOWN_MACRO`, …) keep working without churn.
//!
//! Adding a NEW macro code:
//!   1. Add it to `sdust_diagnostics::codes` (a `DiagCode::new(N)` const
...
/// `sdust_diagnostics::codes::explain` so the catalog stays
