...
//! `sdust-codegen-cranelift::lower` so the two backends ship the same
//! source coverage.
//!
...
struct ProgramLowerer<'ctx, 'a, 'b> {
    ...
    prog: &'a Program,
    user_fns: HashMap<SirFnId, FunctionValue<'ctx>>,
...
impl<'ctx, 'a, 'b> ProgramLowerer<'ctx, 'a, 'b> {
    ...
    fn llvm_ty(&self, t: &SirTy) -> BasicTypeEnum<'ctx> {
        match t {
            SirTy::Bool => self.i8_ty().into(),
            SirTy::Char => self.i32_ty().into(),
            SirTy::Int(k) => match k {
            ...
            SirTy::Duration | SirTy::Size => self.i64_ty().into(),
    ...
    fn fn_type_of(&self, f: &Function) -> inkwell::types::FunctionType<'ctx> {
        let param_tys: Vec<BasicMetadataTypeEnum<'ctx>> = f
            ...
            .filter_map(|p| {
                ...
                if matches!(t, SirTy::Unit | SirTy::Never) {
                    None
                } else {
...
impl<'p, 'ctx, 'a, 'b> FnLowerer<'p, 'ctx, 'a, 'b> {
    ...
    fn lower(&mut self) -> CompileResult<()> {
        ...
        for p in &self.f.params {
            ...
            if matches!(lty, SirTy::Unit | SirTy::Never) {
                continue;
            }
            let arg = self
                .fv
    ...
    fn lower_stmt(&mut self, s: &Stmt) -> CompileResult<()> {
        match s {
            ...
            Stmt::EffectInvoke { op, out, .. } => {
                let method = match op {
                    sdust_sir::sir::EffectOp::GenericCall { method, .. } => method.clone(),
                };
    ...
    fn eval_const(&mut self, c: &Const) -> CompileResult<BasicValueEnum<'ctx>> {
        Ok(match c {
            ...
            Const::Bool(b) => self
                ...
                .into(),
            Const::Int(v, k) => {
                let t = self.pl.llvm_ty(&SirTy::Int(*k));
                match t {
    ...
    fn lower_call(
        ...
        match func {
            ...
            FnRef::User(callee_id) => {
                ...
                let mut callee_param_tys: Vec<SirTy> = callee_fn
                    .params
                    .iter()
