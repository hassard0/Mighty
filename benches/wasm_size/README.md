...
the byte size. The Mighty impl is run by
`crates/mty-bench/src/bin/mty-bench-runner.rs` and recorded in
...
|---|---|---|
| Mighty | `mty build --target wasi --no-component --release synth.sd` | `synth.wasm` |
