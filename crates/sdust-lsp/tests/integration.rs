//! End-to-end-ish tests for the sdust-lsp surface.
//!
...
use sdust_lsp::definition::definition;
use sdust_lsp::diagnostics::build_publish;
use sdust_lsp::docs::{apply_change, DocAnalysis};
use sdust_lsp::hover::hover;
use sdust_lsp::line_index::LineIndex;
...
// ---------------------------------------------------------------------
// formatting (smoke — full coverage lives in sdust-fmt)
// ---------------------------------------------------------------------
...
fn formatting_is_idempotent_for_already_formatted_file() {
    ...
    let formatted2 = sdust_fmt::format(doc2.parsed.green.clone());
