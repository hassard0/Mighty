...
use sdust_types::{AdtKind, DefMap, TyArena, TyData, TyId};
...
mod tests {
    use super::*;
    use sdust_types::{AdtDef, AdtKind, DefMap, FieldDef, TyArena, VariantDef};
    ...
    fn param_var_sendable_conservatively() {
        let mut a = TyArena::new();
        let d = DefMap::default();
        let p = a.param(sdust_types::ParamId(0));
        let v = a.var(sdust_types::TyVarId(0));
        assert!(is_sendable(p, &a, &d));
