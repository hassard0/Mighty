...
use sdust_diagnostics::{codes::*, Diagnostic, Severity};
...
fn assert_emits(name: &str, expected: &[sdust_diagnostics::codes::DiagCode]) {
    let diags = run_negative(name);
    let codes: Vec<sdust_diagnostics::codes::DiagCode> = diags
        .iter()
