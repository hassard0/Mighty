...
/// `cap: &FsCap` parameter of `mty_stdlib::fs::{read, write, exists,
...
fn two_disjoint_caps_isolate_in_the_same_process() {
    ...
    assert_eq!(
        mty_stdlib::fs::read(&cap_a, &a_file).expect("a reads a"),
        b"alpha"
    );
    assert_eq!(
        mty_stdlib::fs::read(&cap_b, &b_file).expect("b reads b"),
        b"bravo"
    );

    // Cross-cap reads are forbidden — cap_a may not see cap_b's file
    // and vice versa.
    let err_ab = mty_stdlib::fs::read(&cap_a, &b_file).expect_err("a must not read b");
    assert!(
        matches!(err_ab, IoErr::Forbidden(_) | IoErr::Denied(_)),
        "expected denial, got {err_ab:?}"
    );
    let err_ba = mty_stdlib::fs::read(&cap_b, &a_file).expect_err("b must not read a");
    assert!(
    ...
    assert!(mty_stdlib::fs::exists(&cap_a, &a_file));
    assert!(!mty_stdlib::fs::exists(&cap_a, &b_file));
    assert!(mty_stdlib::fs::exists(&cap_b, &b_file));
    assert!(!mty_stdlib::fs::exists(&cap_b, &a_file));

    // Writes through cap_a may not land in cap_b's root.
    let illegal = root_b.join("smuggled.txt");
    let write_err = mty_stdlib::fs::write(&cap_a, &illegal, b"nope")
    ...
    // exists on disk.
    let list_err =
        mty_stdlib::fs::list_dir(&cap_a, &root_b).expect_err("list_dir cross-cap must fail");
    assert!(
