...
| Palette commands | `Mighty: Run current file`, `Check current file`, `Build`, `Format`, `Inspect cost` (now a webview), `Inspect cost (terminal)`, `Test --eval`, `Explain diagnostic`, `Restart Language Server`. |
