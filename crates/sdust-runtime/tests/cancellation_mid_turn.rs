...
use sdust_runtime::cancel::{CancelReason, CancellationToken};
...
fn runtime_emits_breach_on_wall_budget_simulated() {
    ...
    // handlers is exercised by sdust-driver conformance tests.
    use sdust_runtime::{RuntimeBuilder, TelemetrySink};
    use std::sync::Arc;
    let src = r#"
protocol P { Hit() -> Str }
agent A: P { on Hit() -> "ok" }
...
fn compile(src: &str) -> std::sync::Arc<sdust_sir::sir::Program> {
