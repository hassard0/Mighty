...
//!
//! Wraps [`serde_json`] but exposes a Mighty-shaped `Json` enum so
...
/// Mighty's `Json` value. Mirrors `serde_json::Value` but uses a
...
pub enum Json {
    Null,
    Bool(bool),
    /// All numerics flow through f64 to match Mighty's surface
