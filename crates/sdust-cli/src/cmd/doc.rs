//! `sdust doc` — Stardust documentation generator. See
//! `docs/reference/cli/sdust-doc.md` for the user-facing reference and
...
pub fn run(
    ...
    let default_name = path
        ...
        .unwrap_or("package")
        .to_string();
    let (doc, diags) = sdust_doc::build_doc_package(&src, &path.to_string_lossy(), &default_name);
    let any_err = diags
        .iter()
    ...
    if html {
        ...
        let files = sdust_doc::render::html(&doc);
        if let Err(e) = sdust_doc::render::write_tree(&dir, &files) {
    ...
    if markdown {
        ...
        let files = sdust_doc::render::markdown(&doc);
        if let Err(e) = sdust_doc::render::write_tree(&dir, &files) {
            eprintln!("sdust doc: write {}: {}", dir.display(), e);
            return 1;
        }
    ...
    if let Some(name) = item {
        if let Some(it) = doc.items.iter().find(|i| i.name == name) {
            print!("{}", sdust_doc::render::item_text(&doc, it));
            0
        } else {
            eprintln!(
        ...
        print!("{}", sdust_doc::render::text(&doc));
        0
    }
}
