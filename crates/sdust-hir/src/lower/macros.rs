...
pub fn check_unknown_macros(source: &str) -> Vec<Diagnostic> {
    let p = sdust_syntax::parse(source);
...
pub fn check_proc_macros(source: &str) -> Vec<Diagnostic> {
    ...
    for child in file.0.children() {
        ...
        let Some(def) = sdust_macros::registry::lower_proc_macro_decl(&child) else {
            continue;
        };
...
fn diag_unknown_macro(start: usize, end: usize, name: &str) -> Diagnostic {
    Diagnostic {
        code: DiagCode::new(sdust_macros::UNKNOWN_MACRO),
        severity: Severity::Error,
        primary: Label {
            start,
            end,
...
fn diag_proc_macro_unsupported(start: usize, end: usize, name: &str) -> Diagnostic {
    Diagnostic {
        code: DiagCode::new(sdust_macros::PROC_MACRO_UNSUPPORTED_V0_5),
        severity: Severity::Error,
        primary: Label {
            start,
            end,
...
fn diag_proc_macro_impure(start: usize, end: usize, name: &str, reason: &str) -> Diagnostic {
    Diagnostic {
        code: DiagCode::new(sdust_macros::PROC_MACRO_IMPURE),
        severity: Severity::Error,
        primary: Label {
            start,
            end,
...
mod tests {
    ...
    fn unknown_bang_macro_emits_sd6001() {
        ...
        assert!(
            pp.diagnostics
                .iter()
                .any(|d| d.code == DiagCode::new(sdust_macros::UNKNOWN_MACRO)),
    ...
    fn proc_macro_call_emits_sd6006() {
        ...
        assert!(
            pp.diagnostics
                .iter()
                .any(|d| d.code == DiagCode::new(sdust_macros::PROC_MACRO_UNSUPPORTED_V0_5)),
    ...
    fn impure_proc_macro_emits_sd6005() {
        ...
        assert!(
            pp.diagnostics
                .iter()
                .any(|d| d.code == DiagCode::new(sdust_macros::PROC_MACRO_IMPURE)),
