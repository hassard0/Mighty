# Stardust stdlib (internals)

The Stardust standard library lives in the `sdust-stdlib` crate. v0.2
ships **Strategy A**: synthesized prelude bindings in `sdust-types`
plus real Rust impls in `sdust-stdlib`, hooked into the SIR
interpreter through `sdust-runtime`'s effect-call sink. v0.3 will
migrate to **Strategy B**: real `.sd` source files shipped as a
bundled package and resolved by `sdust-pkg`.

## Strategy A — how it works

```
+-----------+      +-----------------+      +-------------------+
|  user .sd | ---> | sdust-types     | ---> | SIR EffectOp::    |
...
The runtime can't take a `[dependencies] sdust-stdlib` line without
...
through `sdust-driver`. We side-step the cycle with a process-wide
...
### Wiring `sdust run` to the real stdlib
...
| `std.test` | `sdust_stdlib::test`      | SIR interpreter (via driver)    |
...
`sdust-stdlib` exposes one optional feature:

- `runner` (default-on) — pulls `sdust-driver` + `sdust-diagnostics`
  so `std.test::run_dir` can compile + execute Stardust test files.
...
Strategy B ships `.sd` source files under `crates/sdust-stdlib/src/lib/`
...
1. `sdust-pkg` real module resolution (currently scaffolded).
...
3. Re-targeting each `.sd` file's externs to call the existing Rust
   impls in this crate via the SIR extern table.
