...
pub fn tuple_offset(elems: &[IrTy], idx: usize, adts: &[AdtRef]) -> Option<(u32, Layout)> {
    if idx >= elems.len() {
        return None;
    }
...
/// `IrTy::Error` types are treated as i64 to keep the lowerer total
/// when upstream typeck didn't resolve the binding.
pub fn field_load_ty(t: &IrTy) -> Option<ClType> {
    Some(match t {
        IrTy::Bool => ct::I8,
        IrTy::Char => ct::I32,
        IrTy::Int(k) => match k {
        ...
        IrTy::Float(k) => match k {
            mty_types::FloatKind::F32 => ct::F32,
            mty_types::FloatKind::F64 | mty_types::FloatKind::FloatInfer => ct::F64,
        },
        IrTy::Duration | IrTy::Size => ct::I64,
        IrTy::Str | IrTy::String | IrTy::Bytes => return None, // (ptr, len) pair
        IrTy::Ref { .. } | IrTy::RawPtr(_) | IrTy::Cap { .. } | IrTy::Fn { .. } => ct::I64,
        IrTy::Error | IrTy::Param(_) | IrTy::Module(_) => ct::I64,
        _ => return None,
    })
}

/// Total size, in bytes, of a SIR type (with the ADT catalog).
pub fn type_size(t: &IrTy, adts: &[AdtRef]) -> u32 {
    type_layout(t, adts).size
}

/// Alignment of a SIR type.
pub fn type_align(t: &IrTy, adts: &[AdtRef]) -> u32 {
...
mod tests {
    ...
    fn make_struct(name: &str, field_tys: &[IrTy]) -> AdtRef {
        AdtRef {
            adt: AdtId(0),
            name: name.into(),
    ...
    fn struct_field_offsets_pack_naturally() {
        let s = make_struct("P", &[IrTy::Int(IntKind::I32), IrTy::Int(IntKind::I32)]);
    ...
    fn enum_payload_after_tag() {
        let mut s = make_struct("E", &[IrTy::Int(IntKind::I32)]);
        // Two variants → enum.
    ...
    fn tuple_offsets_pack_naturally() {
        let elems = vec![IrTy::Bool, IrTy::Int(IntKind::I32)];
