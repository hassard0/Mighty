...
//! trusted Rust lexer (`mty_syntax::lex`) and diffs the two token
...
//!
//! After the run, `host.tokens` holds the Mighty lexer's view of the
...
use mty_syntax::{lex as rust_lex, SyntaxKind};
...
fn selfhost_lexer_first_token_matches() {
    ...
    let SelfhostRun { tokens, result } =
        run_selfhost_lexer(input).expect("Mighty lexer compile should succeed");
...
fn selfhost_lexer_full_diff_against_rust() {
    ...
    let input = "fn main() { log(\"hi\") }";
    let SelfhostRun { tokens, result } =
        run_selfhost_lexer(input).expect("Mighty lexer should run");
    assert!(
        matches!(result, RunResult::Ok { .. }),
