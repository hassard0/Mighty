# Mighty Language Specification v1.0 (Release Candidate 2)

**Status:** v1.0-RC2 — released alongside v0.9 (`2026-05-24`,
spec-freeze prep slice). Consolidates the v0.1 base specification
together with 88 numbered amendments (A1..A109) adopted across slices
2..6 (v0.2..v0.6) plus the v0.7 brand rename. RC2 supersedes RC1
(v0.8) with no normative behaviour change; the diff is restricted to:

- Resolutions for the 10 OPEN amendments left by RC1 (3 promoted to
  **FREEZE-MVP**, 7 retained as **DEFER-V1.1** with explicit RFC
  cross-references; 1 — A100 — kept FROZEN with its v1.1+ residual
  tracked via RFC-004).
- Six first-draft RFCs under [`docs/spec/rfcs/`](rfcs/) covering the
  architecturally-significant v1.1+ promotion paths.
- A new "v1.0 stability surface" promotion of §A.1 prose.
- A new "v1.1 promotion targets" cross-reference column in §A.2.
- An "RFCs by amendment" subsection appended to Appendix C.
...
The GitHub repository name is intentionally preserved as
`hassard0/stardust`. The package edition string `"2026"` is a calendar
year identifying the syntactic revision of the language, not a brand
...
The reserved keyword set is fixed for v1.0:
...
The two-token `proc macro` prefix begins a procedural-macro declaration
(see [§20.3](#203-procedural-macros), A94). `proc` itself is **not**
a reserved keyword; the parser recognises the bigram.

**Keyword-tolerant positions** (A3, A4):
- After `.` in expression position, any keyword token is accepted as a
  method or field name. `dom.on("click", h)` and `agent.spawn()`
  parse without raw-identifier escapes.
...
fragment value (see [§22](#22-frontend-model-dom-web)).
...
```
ITEM := USE | MOD | FN | STRUCT | ENUM | TRAIT | IMPL
      | AGENT | PROTOCOL | SUPERVISOR | CAP | SANDBOX
      | EXTERN_BLOCK | MACRO | PROC_MACRO | CONST | STATIC
      | DERIVE | TYPE_ALIAS
```
...
### §26.2 C, C++, JavaScript, Python interop

C interop: via `extern` and the standard C ABI.

C++: name-mangling-aware externs are **v1.1+**.

JavaScript: lowered through the Wasm Component target's `mighty:web/*`
imports. v1.0 DOM surface per [§22](#22-frontend-model-dom-web);
broader JS interop (callbacks, structured data round-trip) is
**v1.1+**.
