...
//!
//! We're measuring Mighty's runtime overhead (accept → parse → write
...
fn bench_http(c: &mut Criterion) {
    ...
    g.bench_function("stardust_http_single_get", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                let d = mty_bench::http::single_get(&addr, "/").await.unwrap();
                black_box(d);
            })
        });
        handle.abort();
    ...
    g.bench_function("stardust_http_seq_100", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                let d = mty_bench::http::sequential_get(&addr, "/", 100)
                    .await
                    .unwrap();
                black_box(d);
            })
