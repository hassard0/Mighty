...
to the MtyIR loop terminator (loops now iterate).
...
hygiene, the lexer is rewritten in Mighty source, and the
long-standing single-iteration loop bug in the MtyIR interpreter is
fixed.
...
mty pkg search rxc
mty pkg info rxc
mty pkg add rxc --version "^0.3"
mty pkg update --refresh   # opt-in network refresh; offline-first by default

# Publish a deterministic bundle to GitHub Releases.
SDUST_PKG_LOGIN_TOKEN=ghp_… mty pkg login
mty pkg publish my_package

# Declarative macros now expand with hygiene.
# (see examples/16_macro.sd; mty-macros::diag exports MT6001..MT6004)
mty check examples/16_macro.sd

# The Mighty lexer is now written in Mighty source.
mty check selfhost/lexer/lexer.sd

# Loops actually loop.
mty run examples/06_for_while_loop.sd       # main() is still log-only, but
...
| pkg registry | `mty-pkg` rewrite + new CLI subcommands + docs | `fb91aea`, `058b027` |
| proc / declarative macros | `mty-macros` (new crate) + `mty-hir/src/lower/macros.rs` | `c636dbc`, prep `69de965` |
| self-host lexer | `selfhost/lexer/lexer.sd`, `lib.sd`, `syntax_kind.sd`, `crates/mty-driver/tests/selfhost_lexer.rs`, `docs/internals/self-hosting.md` | `913d15c`, `86e3d67`, `53b4673` |

Plus one substantive integration-time change: the MtyIR loop
terminator fix in `crates/mty-sir/src/lower/exprs.rs`.
...
- **20 crates** in the workspace (was 19; `mty-macros` added)
...
| Mighty source can express its own lexer | external Rust only | subset compiles + first-token round-trips via `std.io` host bridge (A63) |
...
- **Procedural / declarative macros** — declarative-side shipped (mty-macros: expansion + mangling-based hygiene + MT6001..MT6004; proc macros remain v0.5+)
- **Real `loop { break }` lowering** — partially shipped (the MtyIR loop terminator is fixed; `break`/`continue` HIR nodes remain v0.5)
...
A62 — MtyIR loop terminator routes to header (loops iterate; `break` still v0.5)
...
`mty explain SD6xxx` is not yet wired (v0.5 cleanup folds these
codes into `mty-diagnostics::codes`).

## Toolchain

- **MSRV: Rust 1.85** (unchanged from v0.2)
- New crate `mty-macros` in the workspace (no new optional
  features — pure library, plumbed in via `mty-hir`)
- `mty-pkg` gains deps on `tar`, `flate2`, `dirs`, `serde_json`
...
- **Registry / pkg**: create + seed the default registry repo;
  consolidate `Manifest` into `mty-pkg`; bundle
...
- **Demos**: `std.http.serve` through the dispatcher + Handler
  adapter; `mighty:web/dom` import lowering; MtyIR-side
  auto-charging for cpu/mem budget caps.
- **Carried from v0.3**: Polonius-style borrows, slice-7
  supervisor / cap-narrow strict resolution, MtyIR-side
...
1. **`break` / `continue` are not yet HIR nodes** — they parse
   ...
   contract).
2. **`for` has no iterator-exhaustion check in the MtyIR
...
5. **`mac!name(...)` syntactic marker not yet parsed** — MT6001
   stays reserved.
6. **No default registry** — `mighty-pkg/registry` is reserved
   but not created. Network fetches surface a clean 404.
7. **Plaintext auth token** at `~/.config/mty/auth.toml`.
...
- **Loops iterate.** Code that quietly relied on the v0.3 MtyIR
...
- **Single-file packages picked up new resolver paths.** `add` /
  `update` now look in `~/.cache/mty/registry/` (or the
...
- **`registry+https://...` URL prefix is rejected.** Migrate
  lockfiles by running `mty pkg update` — the error message
  spells out the migration.
- **`mty pkg publish` now produces a deterministic `tar.gz`.**
...
the three new `mty pkg` subcommands (`search` / `info` /
`login`) and `pkg update --refresh`.

## Acknowledgments

v0.4 is the third Mighty release built by autonomous parallel
agents. The four swarm agents shipped tightly because each
touched disjoint crates — demos (no compiler edits) vs pkg vs
new mty-macros crate vs selfhost — and the integrator only
needed to apply the MtyIR loop terminator fix (plus a one-test
update to acknowledge the new iterative semantics). The agents
stood on the slice-1..8 + v0.2 + v0.3 foundations: the
declarative parser, the typed HIR / MtyIR / interpreter, the
