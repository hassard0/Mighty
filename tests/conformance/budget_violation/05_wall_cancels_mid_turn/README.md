...
The case still flows through the MtyIR interp in the conformance
harness (the harness is mty-driver based and doesn't spin up
`mty-runtime`), so the deadline-fire path here is tested at the
runtime level in `crates/mty-runtime/tests/cancellation_mid_turn.rs`.
