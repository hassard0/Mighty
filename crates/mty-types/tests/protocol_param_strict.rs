...
//! declared param type. Mismatch fires MT4031.
//!
//! For **external** protocols (e.g. `http.Handler` in another module),
//! the check is skipped and MT2026 is emitted as before.
...
fn diag_codes(src: &str) -> Vec<String> {
    let parsed = parse_source(src.into(), "protocol_param_strict.mty".into());
...
fn local_protocol_param_type_mismatch_fires_sd4031() {
    ...
    assert!(
        codes.contains(&"MT4031".to_string()),
        "expected MT4031 for local-protocol param-type mismatch, got {:?}",
        codes
    );
}
...
fn local_protocol_param_type_matching_is_clean() {
    ...
    assert!(
        !codes.contains(&"MT4031".to_string()),
        "matching param types should not raise MT4031, got {:?}",
        codes
    );
}
...
fn external_protocol_keeps_sd2026_warning_path() {
    ...
    // The handler walks via the Unknown lookup → MT2026 warning, NOT
    // MT4031. This preserves v0.2 example 19's behavior.
    let src = r#"
        use std.http
    ...
    // MT2026 warning should be present; MT4031 must NOT be present.
    assert!(
        codes_all.contains(&"MT2026".to_string()),
        "external protocol should warn MT2026, got {:?}",
        codes_all
    );
    let errors_only = diag_codes(src);
    assert!(
        !errors_only.contains(&"MT4031".to_string()),
        "external protocol must NOT fire MT4031, got {:?}",
        errors_only
    );
}
