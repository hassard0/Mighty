...
//!
//! sdust-syntax uses rowan's CST which keeps trivia inline. For macro
...
impl Tok {
    ...
    /// `IDENT`s in well-formed Stardust source.
...
/// string that sdust-syntax's [`parse_expr`](sdust_syntax::parse_expr)
/// can consume.
...
pub fn lex_fragment(src: &str) -> Option<Vec<Tok>> {
    let lexed = sdust_syntax::lex(src);
