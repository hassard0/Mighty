...
use mty_ir::ir::{AgentIrId, Program};
...
pub fn run_one_turn_with_shared_reply(
    ...
    let (rr, new_state, reply_value) = mty_ir::interp::run::run_handler_isolated(
        prog,
        handler_fn_id,
        state_in,
        payload_args,
    ...
    match rr {
        ...
        mty_ir::interp::run::RunResult::Trap { code, message } => {
        ...
        mty_ir::interp::run::RunResult::BudgetExceeded => {
        ...
        mty_ir::interp::run::RunResult::MemBudgetExceeded { used, limit } => {
        ...
        mty_ir::interp::run::RunResult::NoMain => {
            let err = RuntimeError::HandlerNotFound {
                agent: desc.name.clone(),
                msg: proto_msg.clone(),
            };
