...
fn bundle_re_run_produces_identical_sha256() {
    let dir = tempfile::tempdir().unwrap();
    write_fixture(dir.path());
    let a = sdust_pkg::publish::bundle(dir.path()).unwrap();
    let b = sdust_pkg::publish::bundle(dir.path()).unwrap();
...
fn bundle_extraction_round_trips_through_fetcher() {
    ...
    sdust_pkg::fetch::registry::extract_targz(&bytes, dst.path()).unwrap();

    let unpacked_root = dst.path().join("fixture-0.4.0");
    assert!(unpacked_root.join("star.toml").exists());
