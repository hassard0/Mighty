...
fn c_arena_block() {
    assert_snapshot!(dump_expr(
        "arena turn { let toks = tokenize(input); parse(toks)? }"
    ));
}
#[test]
fn c_task_scope() {
    assert_snapshot!(dump_expr(
        "task scope { let a = spawn task fetch(a_url); join a? }"
    ));
}
#[test]
fn c_task_scope_deadline() {
...
fn c_budget() {
    assert_snapshot!(dump_expr(
        "budget { cpu 150ms wall 2s mem 128MiB mb 1k } run job(input)?"
    ));
}
#[test]
fn c_sandbox() {
