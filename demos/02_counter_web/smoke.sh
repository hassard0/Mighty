...
# 1) check + build
"$SDUST" check "$ROOT/demos/02_counter_web/src/main.sd" >/dev/null
"$SDUST" build --target wasm32-web --out-dir "$OUT" \
    "$ROOT/demos/02_counter_web/src/main.sd" >/dev/null
...
if ! grep -aFq "stardust:web/log" "$WASM"; then
  echo "smoke FAIL: 'stardust:web/log' import not found in component" >&2
  exit 1
fi
...
# wasm targets).
host_out="$("$SDUST" run "$ROOT/demos/02_counter_web/src/main.sd" 2>&1)"
