...
## A107 — Central diagnostic catalog for SD6001-SD6006 (v0.6)

The macro-band diagnostic codes (SD6001 `unknown_macro` through
SD6006 `proc_macro_unsupported_v0_5`) live in
`sdust_diagnostics::codes` as `DiagCode` constants alongside every
other Stardust diagnostic. `sdust_macros::diag` keeps the historical
bare-`u16` constants but re-exports their values from the catalog;
adding a new SD6xxx now means adding it to one file
(`sdust-diagnostics/src/codes.rs`) and re-exporting the `u16` from
`sdust-macros/src/diag.rs`. `sdust explain SDxxxx` is single-sourced
on the catalog and now resolves SD6001-SD6006 the same way it
resolves every other code.

## A108 — `BuiltinId::DomOp(name)` SIR variant (v0.6)

`sdust_sir::sir::BuiltinId` gains a `DomOp(String)` variant carrying
the bare SIR method name (`set_text`, `get_text`, `on_click`,
...
routes DomOp through `emit_dom_call` to the four `stardust:web/dom`
imports; the SIR interpreter routes it through
...
the sandbox manifest at the SIR lower remains v0.7 work; the
