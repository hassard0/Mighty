...
3. Coherence: if `(trait, self_adt)` already exists in
   `defs.traits.impl_keys`, emit `SD4022 trait_coherence_violation`.
...
2. **Trait fallback**: collect `defs.traits.by_method[(aid, method)]`.
   ...
   `SD4020 method_ambiguous` listing trait candidates.
3. **Neither** → fall through to the slice-4 built-in table or, for
   user (non-opaque) ADTs, emit `SD4021 method_not_found` /
   `SD2007 unknown_method` (kept for back-compat).
...
`has_generics`, emit `SD4023 dyn_requires_object_safe`. (Slice 5
...
(walks `is_field_copy`); failure → `SD4040 derive_copy_field_not_copy`.
