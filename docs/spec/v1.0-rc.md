# Mighty Language Specification v1.0 (Release Candidate)

**Status:** v1.0-RC — released alongside v0.8 (`2026-05-24`). Consolidates the
v0.1 base specification together with 88 numbered amendments (A1..A109)
adopted across slices 2..6 (v0.2..v0.6) plus the v0.7 brand rename.

**Predecessor:** [`docs/spec/v0.1.md`](v0.1.md) (stub) +
[`docs/spec/v0.1-amendments.md`](v0.1-amendments.md) (88 amendments with
per-amendment `**Status:**` classifications).

**Change log:** [`docs/spec/CHANGELOG.md`](CHANGELOG.md).

**Scope decisions:** [`SPEC_CONSOLIDATION_V0_8_NOTES.md`](../../SPEC_CONSOLIDATION_V0_8_NOTES.md).
...
- **Anonymous error unions** (A11) — currently resolve to
  `Result[T, Error]` sentinel. v1.1+: first-class union ADTs.
- **Arena escape detection** (A15, A31) — direct-naming MVP only.
  v1.1+: indirect-flow detection + MT5007 runtime trap.
- **Permissive method table** (A10) — kept for opaque prelude ADTs.
  v1.1+: typed inherent/trait dispatch removes residual permissive
  table.
- **`--legacy-interp` flag** (A45) — retained pending v1.1+
  deprecation once codegen covers the full MtyIR surface.
- **Wasm Component Model wrapper** (A47) — core modules only in v1.0.
  v1.1+: `wit-component` + WIT auto-binding + preview2/wasi-cli.
- **Per-(fn, type-args) monomorphisation** (A49) — strips generics in
  v1.0 MVP. v1.1+: full specialisation.
- **Canonical-ABI DOM return bridge** (A97) — `get-text` and `query`
  return `u32` handles in v1.0. v1.1+: real `string` / `option<string>`
  returns.
- **Agent affinity front-end syntax** (A102) — runtime API only in
  v1.0. v1.1+: `agent X with affinity = sticky` parser support.
- **Lossless live agent migration** (A103) — routing-table-only in
  v1.0. v1.1+: in-flight loop migration.
- **Proc-macro execution** (A94) — parse + store only in v1.0,
  MT6006-gated at every call site. v1.1+: sandboxed execution.
- **Per-call FsCap manifest materialisation** (A100) — process-wide
  default cap stop-gap in v1.0. v1.1+: MtyIR lower threads per-call
  cap from sandbox manifest.
- **Per-agent HTTP dispatch wiring** (A96) — default-echo dispatcher in
  v1.0. v1.1+: real per-agent routing through
...
*End of Mighty Language Specification v1.0 (Release Candidate).*
