# Mighty stdlib v0.2 — implementation notes

This file documents the choices made while implementing `mty-stdlib`
...
For `mty run` to actually route `std.json.parse` through the real
...
Until then, `mty run` programs that call `std.*` see a `Value::Unit`
...
`mty-stdlib` exposes a `runner` feature (default-on) that pulls
`mty-driver` + `mty-diagnostics` for the `std.test` test-runner.
...
- Wraps `serde_json` but exposes a Mighty-shaped `Json` enum with a
...
- Execution: parse → HIR-lower → type+borrow check → MtyIR-lower →
...
- **v0.3 plan**: replace the `test_` prefix convention with a real
  ...
  scope for the wave-2 slice (touches mty-syntax/mty-hir).
- Ships as a standalone `mty-test` binary; v0.3 merges it into the
  main `mty` CLI as `mty test`.

## Build-time gotcha — workspace race

This slice ran in parallel with two other agents (Wasm CM, DWARF) that
were modifying `crates/mty-codegen-cranelift/` and
`crates/mty-codegen-wasm/` simultaneously. While the parallel work
was uncommitted, the workspace build for `mty-stdlib --features runner`
...
```bash
cargo test -p mty-stdlib --no-default-features
```

Once the parallel agents land their slices, the full
`cargo test -p mty-stdlib` (runner feature on) goes green too.
