...
Plus several slice-4 polish items: `move *ref` modelling for MT3009,
tighter MT3002 vs MT3008 distinction, and (stretch) field-level borrows.
...
- Every `pub fn`'s declared `effect` clause is a superset of the body's
  inferred effects (else `MT4001 effect_undeclared`).
...
- A function consuming `fs: FsReadOnly[Path("/data")]` cannot pass it to
  a fn requiring a broader `Fs` capability (`MT4010 capability_too_broad`).
- `impl Trait for Type { ... }` registers the trait/method pair in a
  coherence table; `let h: dyn Hash = user_id` requires the
  `UserId: Hash` impl in scope (`MT4023 dyn_requires_object_safe`,
  `MT4020 method_ambiguous`, `MT4021 method_not_found`).
...
- For every `pub fn` with an `effect ...` clause, the declared set MUST be
  ...
  `MT4001 effect_undeclared` listing the missing effects.
- For every `pub fn` *without* an `effect` clause, the inferred set must
  ...
  profile). If non-empty, emit `MT4001` recommending the clause.
...
- Default profile in slice 5 is `host` (permissive). The 20 examples use
  the host profile, so no example needs source changes for MT4002.
...
3. At call sites where a parameter's type is a `Cap { family: F, constraint: C }`
   ...
   `MT4010 capability_too_broad`. The "broader argument" case (caller
...
4. Path-`as` syntax (`fs as Fs`) explicitly broadens — also illegal in
   slice 5; emit `MT4010` and recommend `fs.path(...)` instead. Slice 5
...
2. Else if receiver is a user `Adt(aid, args)`:
   ...
   b. **Trait impls in scope**: `trait_table.by_method[(aid, "m")]` may
      ...
      - Two or more → `MT4020 method_ambiguous`. Recommend type-annotated
        call: `<T as Trait>.m(receiver)`. (Slice 5 doesn't parse this
        syntax; just reports the candidates.)
   c. Neither → `MT4021 method_not_found`. (Replaces slice-4's
...
- For each `impl Trait for T { ... }`, check `trait_table.impl_keys.contains(&(trait, T))`.
  If yes, emit `MT4022 trait_coherence_violation`. The check is
...
2. The RHS expression's type must be `Adt(T, ...)` such that the trait
   table has `(Hash, T) → impl`. If no impl: `MT4023 dyn_requires_object_safe`
...
MT2026. Slice 5 promotes the strict checks:

- **MT4030 protocol_arity_mismatch**: `on Msg(p1)` where the protocol
  declares `Msg(p1: Str, p2: U32)`. Error.
- **MT4031 protocol_param_type_mismatch**: the protocol declares
  ...
  diagnostic is the normal `MT2001` mismatch surfaced from binding.
  Slice 5 promotes the binding mismatch (param annotation provided
  vs body usage) to `MT4031` for clearer messaging.
- **MT4032 protocol_missing_handler**: the agent implements `Protocol A`
  which declares messages `M1, M2`, but only `on M1` is provided. Error.
- **MT4033 protocol_extra_handler**: `on UnknownMsg(...)` for a name not
  in any implemented protocol. Error (slice 4 was a warning MT2026).
...
arity + coverage. Replace the slice-4 warning with the new MT4032/33
errors.

### 3.10 MT3009 `move *ref` proper modelling

Today `HirExpr::Move(HirExpr::Unary { op: Deref, ... })` flows through
the walker without special handling. Slice 5 adds: if the moved
expression is a deref of a reference, emit `MT3009 move_out_of_ref`.

### 3.11 Tighter MT3002 vs MT3008 distinction

MT3002 ("move out of borrow") fires when the borrowed value itself is
moved while a borrow lives. MT3008 ("cannot move borrowed") is the
same condition — slice 4 used MT3002 universally. Slice 5 distinguishes:
- MT3002: the *original owner* moves while a borrow holds.
- MT3008: a third party tries to move via a non-owner path.
Slice 5 implementation: MT3008 triggers when the move expression is in
function-argument position; MT3002 triggers on direct let/return moves.
...
## 4. Diagnostics (MT4001..MT4099)

| Code   | Name                          | Severity | Example                                                          |
|--------|-------------------------------|----------|------------------------------------------------------------------|
| MT4001 | effect_undeclared             | Error    | `pub fn f() { net.get(url) }` (no `effect net` declared)         |
| MT4002 | alloc_in_core                 | Error    | strict-profile fn allocates via `arena` or container             |
| MT4010 | capability_too_broad          | Error    | callee wants `Fs (ReadOnly, /data)`, caller provides `Fs (Any)`  |
| MT4020 | method_ambiguous              | Error    | `x.hash()` where two traits in scope provide `hash` for `T`      |
| MT4021 | method_not_found              | Error    | `x.fly()` no impl in inherent / trait tables                     |
| MT4022 | trait_coherence_violation     | Error    | `impl Hash for U` declared twice                                 |
| MT4023 | dyn_requires_object_safe      | Error    | `let h: dyn Hash = x` and `Hash` has `Self`-returning method     |
| MT4030 | protocol_arity_mismatch       | Error    | handler has 1 param, protocol message has 2                      |
| MT4031 | protocol_param_type_mismatch  | Error    | handler param used as Int, protocol declares Str                 |
| MT4032 | protocol_missing_handler      | Error    | implements `Fetch (Page, Head)` but only `on Page`               |
| MT4033 | protocol_extra_handler        | Error    | `on UnknownMsg` not in any implemented protocol                  |
| MT4040 | derive_copy_field_not_copy    | Error    | `#[derive(Copy)] struct S { v: String }`                          |
| MT4041 | derive_unknown                | Error    | `#[derive(Foo)]` where `Foo` is not a known derivable trait      |
...
- **A26**: Derive set for slice 5 — `Copy`, `Hash`, `Eq`. Other derive
  names emit `MT4041 derive_unknown`.
...
- **A29**: `move *ref` is now MT3009 (was permissive in slice 4).
...
7. Trait coherence table + dispatch upgrade (MT4020/21/22).
