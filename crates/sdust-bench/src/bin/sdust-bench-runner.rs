//! `sdust-bench-runner` — orchestrate the v0.6 benchmark categories and
//! emit a JSON summary that the doc scripts can ingest.
//!
//! For each category, we run the **Stardust impl in-process** (no
//! criterion harness — that's what `cargo bench -p sdust-bench` is for)
...
//!
//! ```text
//! sdust-bench-runner --category parse_throughput --iters 50
//! sdust-bench-runner --all --iters 30 --out target/bench-results.json
//! ```
...
#[derive(Parser, Debug)]
#[command(name = "sdust-bench-runner", version)]
struct Args {
...
fn run_parse(iters: usize) -> Sample {
    ...
    for _ in 0..iters {
        let s = src.clone();
        let t0 = Instant::now();
        let parsed = sdust_driver::parse_source(s, "synth.sd".into());
...
fn run_http(iters: usize) -> Sample {
    ...
    for _ in 0..iters {
        let dur = rt.block_on(async {
            ...
            let elapsed = sdust_bench::http::single_get(&addr, "/").await.unwrap();
            h.abort();
            elapsed
        });
        samples.push(dur);
...
fn run_compile(iters: usize) -> Sample {
    ...
    use sdust_driver::{build_wasm, BuildOptions, BuildTarget};
    let src = sdust_bench::fixtures::synth_source(100); // ~1 KLOC
    let mut samples = Vec::with_capacity(iters);
    for i in 0..iters {
    ...
    finish(
        ...
        Some(serde_json::json!({
            "lines": sdust_bench::fixtures::synth_source(100).lines().count(),
...
fn run_wasm_size() -> Sample {
    use sdust_codegen_cranelift::artifact::BuildMode;
    use sdust_codegen_wasm::WasmTarget;
    use sdust_driver::{build_wasm, BuildOptions, BuildOutcome, BuildTarget};
    let src = sdust_bench::fixtures::synth_source(50);
    let tmp = tempfile::tempdir().unwrap();
    let opts = BuildOptions {
        target: BuildTarget::Wasm(WasmTarget::Wasi),
        mode: BuildMode::Release,
