...
A clickable counter rendered into a browser, backed by a Mighty agent
...
> `mighty:web/dom` interface (`set-text`, `get-text`, `on-click`,
> `query`) — see `crates/mty-codegen-wasm/src/wit.rs`. The
...
./target/debug/mty check demos/02_counter_web/src/main.sd
...
through the `mighty:web/log#log` import; the JS host parses the log
...
demonstration loader, not a production pattern — the moment Mighty
adds another host import (DOM bindings, networking) the canonical
`jco` flow takes over.

The Mighty wasm32-web backend wires the `log` import end-to-end
(see `crates/mty-codegen-wasm/src/wit.rs`); the WIT *stubs* DOM
