use crate::nodes::*;
use crate::ids::*;
use sdust_ast::{AstNode, FnDecl, StructDecl, EnumDecl, TypeAlias, AgentDecl, ProtocolDecl, SupervisorDecl, UseDecl, ModDecl};
...
fn lower_fn(ctx: &mut LoweringCtx, f: FnDecl) -> FnId {
    ...
    let params = f.param_list().map(|pl| {
        pl.0.children().filter_map(sdust_ast::FnParam::cast).map(|p| {
            ...
            let ty = p.0.children().find(|c| is_type_node(c.kind()))
