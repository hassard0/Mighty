...
//! backed registry.
//!
//! A "Stardust registry" is a GitHub repository whose Releases host
...
/// The official Stardust registry repository slug. The cloud control
...
/// `sdust-driver`) so this slice doesn't have to touch the driver.
...
impl AuthStore {
    ...
    pub fn default_path() -> Option<PathBuf> {
        dirs::config_dir().map(|p| p.join("sdust").join("auth.toml"))
    }
