...
//!
//! Runs the Stardust parser in `selfhost/parser/parser.sd` over a canned
...
//! Rust parser (`sdust_syntax::parse`) and diffs the two trees BFS:
...
//! `SelfhostParserHost`; the Stardust source is the pure algorithm.
//!
...
use sdust_syntax::{lex as rust_lex, parse as rust_parse, SyntaxKind, SyntaxNode};
...
impl SelfhostParserHost {
    fn seed(&mut self, src: &str) {
        ...
        // Note: the Stardust parser will see ALL tokens including trivia
        ...
        match child {
            ...
            rowan::NodeOrToken::Token(t) => {
                ...
                if tk.is_trivia() {
                    ...
                    // capture them as leaves to match the Stardust side,
...
/// inside the surrounding node; the Stardust parser tends to emit it
...
fn selfhost_parser_hello_world() {
    // Parses `fn main() { log("hi") }` via the Stardust parser source
    ...
    let SelfhostRun { events, result } =
        run_selfhost_parser(input).expect("Stardust parser should compile");
    assert!(
...
fn selfhost_parser_event_protocol_smoke() {
    // The Stardust parser opens a FILE node, walks through the (empty)
