...
**Implementation status.** Parsing of row variables shipped in v0.15
(`crates/mty-syntax::parser::types::effect_clause`); the resulting CST
exposes new `EFFECT_SET`, `EFFECT_NAME`, `EFFECT_ROW_TAIL`, and
`EFFECT_ROW_VAR` node kinds. **Full type-checker wiring lands in v0.16
(post-v1.0)**: the v0.16 work adds a `HirEffectRow` HIR node, plumbs
it through `mty-hir::lower::items::lower_fn`, and constructs
`RowPolySig` in `mty-types::check.rs` so call sites unify the row
variable per RFC-008's four-case unification table. Until v0.16
ships, the parser emits the new shape but the HIR lowerer discards
the row tail; semantically the fn behaves as if its declared row
were `Closed(concrete_effects)`. Diagnostics MT4020–MT4025 from
RFC-008 §Diagnostics also land in v0.16.
