...
//! Pipeline:
//!
//! 1. Run [`mty_syntax::parse`] + [`mty_hir::lower`] on the input
//!    source. (We deliberately avoid `mty-driver` to keep this crate
...
pub fn build_doc_package(
    ...
    let (pkg, lower_diags) = mty_hir::lower::LoweringCtx::new().lower_file(file.clone());
...
fn doc_from_mod(node: &SyntaxNode, raw: &str) -> Option<DocModule> {
    let path: Vec<String> = node
        .children()
        .find_map(mty_ast::Path::cast)
        .map(|p| {
            p.0.children()
                .filter_map(mty_ast::PathSegment::cast)
                .map(|s| {
                    s.0.first_token()
...
fn file_package_name(file: &File) -> Option<String> {
    ...
    let path = pd.children().find_map(mty_ast::Path::cast)?;
