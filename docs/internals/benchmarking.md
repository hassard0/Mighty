...
│   └── bin/
│       └── sdust-bench-runner.rs     # CLI driver (criterion-less)
├── benches/
...
|---|---|
| Pure-Stardust benchmark | `crates/sdust-bench/benches/<name>.rs` |
| Shared fixture / helper | `crates/sdust-bench/src/<name>.rs` |
...
`crates/sdust-runtime/benches/`). Use the top-level `benches/` only
for **cross-language comparative** benchmarks; per-crate benches are
fine for in-language tuning.

## Adding a new category

1. **Add a shared fixture** in `crates/sdust-bench/src/fixtures.rs` if
   you need new test data.
2. **Add the criterion bench** at
   `crates/sdust-bench/benches/<name>.rs` with `harness = false`
   in `Cargo.toml`. Use `BenchmarkId::new` to keep the report
   structure consistent.
3. **Add the runner integration** in
   `crates/sdust-bench/src/bin/sdust-bench-runner.rs` by extending
...
./target/release/sdust-bench-runner --all --iters 30 --out target/bench.json
...
("Stardust is 1.4x slower than Rust at median") over **mean ratios**
which are noisy.
...
- **Don't cherry-pick the best run.** Run 20-30 iterations and report
  the median.
- **Don't omit losing benchmarks.** If Stardust is 5x slower than
