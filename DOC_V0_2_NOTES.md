...
`Manifest` loader is intentionally not pulled in (`sdust-doc` deps
were pared to avoid the `sdust-driver` → `sdust-codegen-cranelift`
chain, which was in-flight when this crate landed).

> v0.3: thread the package version from `Stardust.toml` via a new
...
`sdust check`. Example bodies are surfaced verbatim in markdown and
HTML output.
...
by spec convention in v0.2 — see `crates/sdust-hir/src/nodes.rs`). The
...
`crates/sdust-doc/src/extract.rs::render_type`. It does not consult
`sdust_fmt::printer` — partly because `sdust-fmt`'s type printer
...
> `sdust-types` or `sdust-fmt` and share it.

## 7. HTML uses an inline template, not Askama

Askama 0.12 is declared as a workspace dep (and as a `sdust-doc` dep)
...
There is no `sdust doc --check` to verify a documented API hasn't
