...
pub fn lower_pat(ctx: &mut LoweringCtx, n: SyntaxNode) -> PatId {
    let p = match n.kind() {
        ...
        SyntaxKind::BINDING_PAT => {
            ...
            let sub = n
                .children()
                .find(|c| is_pat_node(c.kind()))
                .map(|sn| lower_pat(ctx, sn));
        ...
        SyntaxKind::STRUCT_PAT => {
            ...
            let fields = n
                .descendants()
                .filter(|c| {
                    c.kind() == SyntaxKind::IDENT_PAT || c.kind() == SyntaxKind::BINDING_PAT
                })
                .map(|f| {
                    let nm = f
                        .children()
        ...
        SyntaxKind::RANGE_PAT => {
            ...
            let inclusive = n
                .children_with_tokens()
                .filter_map(|e| e.into_token())
                .any(|t| t.kind() == SyntaxKind::DOT_DOT_EQ);
