...
//! [`sdust_debuginfo::DwarfBuilder`] from a SIR program + the original
...
use sdust_sir::sir::{Function, LocalSource, Program, SirTy};
...
pub fn display_ty(t: &SirTy) -> String {
    match t {
        SirTy::Bool => "bool".into(),
        SirTy::Char => "char".into(),
        SirTy::Str => "str".into(),
        SirTy::String => "String".into(),
        SirTy::Bytes => "Bytes".into(),
        SirTy::Unit => "()".into(),
        SirTy::Never => "!".into(),
        SirTy::Duration => "duration".into(),
        SirTy::Size => "size".into(),
        SirTy::Int(k) => format!("{k:?}").to_lowercase(),
        SirTy::Float(k) => format!("{k:?}").to_lowercase(),
        SirTy::Tuple(elems) => {
            let parts: Vec<_> = elems.iter().map(display_ty).collect();
            format!("({})", parts.join(", "))
        }
        SirTy::Array { elem, len } => match len {
            Some(n) => format!("[{}; {}]", display_ty(elem), n),
            None => format!("[{}]", display_ty(elem)),
        },
        SirTy::Ref { mutable, inner } => {
            if *mutable {
        ...
        SirTy::Adt(_, _) => "adt".into(),
        SirTy::Cap { family, .. } => format!("Cap<{family:?}>"),
        SirTy::Dyn(name) => format!("dyn {name}"),
        SirTy::RawPtr(inner) => format!("*const {}", display_ty(inner)),
        SirTy::Module(name) => format!("module<{name}>"),
        SirTy::Param(name) => name.clone(),
        SirTy::Error => "error".into(),
    }
}
...
mod tests {
    ...
    use sdust_sir::sir::{Block, BlockId, Const, Function, LocalDecl, Operand, SirFnId, Term};
    ...
    fn display_ty_examples() {
        assert_eq!(display_ty(&SirTy::Bool), "bool");
        assert_eq!(display_ty(&SirTy::Int(IntKind::I32)), "i32");
        assert_eq!(
            display_ty(&SirTy::Tuple(vec![SirTy::Bool, SirTy::Int(IntKind::I32)])),
            "(bool, i32)"
        );
    }
}
