...
//!
//! A procedural macro is a Stardust function of shape
...
//! Execution is deferred to v0.6 because running a Stardust function at
...
pub fn check_proc_macro_purity(body: &[Tok]) -> Option<ImpurityReason> {
    ...
    // `effect`, which is itself a keyword in Stardust — so we also
    // match EFFECT_KW.
    let mut i = 0;
    while i < body.len() {
