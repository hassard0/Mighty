...
`sdust-codegen-wasm` translates SIR into a Component Model component
...
[A47](../spec/v0.1-amendments.md#a47)**: `sdust build --target
...
```
SIR Program ─► core wasm bytes ─┐
                                ├─► embed `component-type` ─► ComponentEncoder ─► component .wasm
generated WIT ──────────────────┘
```

1. `compile_program_to_bytes` lowers SIR → core Wasm (slice-8 path).
...
from the SIR program. The shape is:
...
world <pkg-name>-world {
  import wasi:cli/log;          // or stardust:web/log for web target
  import stardust:caps/fs;       // for each cap family used in the program
...
package stardust:caps { ... }     // bundled stubs so wit-parser is satisfied
package wasi:cli { ... }
```
...
| `Cap<family>` param | `import stardust:caps/<family>;` |
