...
worker runtime. The SIR case here just validates the loop+ask shape
deterministically. The companion Rust integration test
`crates/sdust-runtime/tests/multicore_throughput_smoke.rs` runs **4
...
`sdust-bench` crate (criterion benches).
