...
**Status:** OPEN - Anonymous error unions still resolve to Result[T, Error] sentinel; first-class union ADTs deferred to v1.1+.
...
**Status:** OPEN - Arena escape direct-naming MVP; indirect-flow detection still post-v1.0.
...
**Status:** OPEN - Arena runtime enforcement still partial — MT5007 reserved; static MT3010 + bumpalo (A50) cover most cases.
...
**Status:** OPEN - --legacy-interp flag retained; deprecation/removal deferred to v1.1+ once codegen covers full MtyIR surface.
...
**Status:** OPEN - Wasm Component Model wrapper deferred; core modules ship today. A97 (mighty:web/dom interface) layered on top; full wit-component still v1.1+.
...
**Status:** OPEN - Per-(fn, type-args) monomorphisation strips generics in v0.1 MVP; full specialisation in v1.1+.
...
**Status:** OPEN - Procedural macros parse + store + purity check (MT6005/MT6006); sandboxed execution deferred to v1.1+, MT6006 gates every call site today.
...
**Status:** OPEN - mighty:web/dom interface added; canonical-ABI return-area bridge for option<string> / string returns still v1.1+.
...
**Status:** FROZEN - FsCap process-wide default allowlist enforcement shipped v0.5; per-call materialisation from manifest still v1.1+ (A109 pins isolation contract).
...
**Status:** OPEN - Affinity hints: runtime API frozen, front-end syntax `agent X with affinity = ...` reserved-but-not-parsed pending v1.1+.
...
**Status:** OPEN - Lightweight migration shipped; lossless live migration deferred to v1.1+.
