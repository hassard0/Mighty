...
**Workload:** lex + parse a 10 002-line synthetic Stardust source
(~130 KB; deterministic — see `crates/sdust-bench/src/fixtures.rs`).
...
|---|---|---|---|---|
| Stardust v0.6 (full pipeline) | 6.19 ms | 10.01 ms | 10.40 ms | parse_source: lex + CST + diag collection |
| Stardust v0.6 (lex only) | (see criterion HTML) | | | logos pass only |
...
The Stardust parse pipeline is logos-backed + a hand-written
...
We expect a ~3-5x gap between Stardust's full pipeline and a bare
