# Mighty Slice 8 — Complete (FINAL v0.1 slice)
...
```
crates/mty-codegen-cranelift/    ~1 200 lines
crates/mty-codegen-wasm/         ~  600 lines
crates/mty-codegen-llvm/         ~   50 lines (scaffold, feature-gated)
```

### Cranelift native backend (spec §24.6, A46)

- **JIT path** (`mty run` default): MtyIR → cranelift IR → in-process
  fn-pointer, called via transmute. Runtime ABI bridge
  (`sdust_runtime::codegen_abi`) supplies twelve C-ABI fns.
- **AOT path** (`mty build --target native`): cranelift `ObjectModule`
...
- **Type lowering** ([`abi`](crates/mty-codegen-cranelift/src/abi.rs))
  + **layout** ([`layout`](crates/mty-codegen-cranelift/src/layout.rs))
...
- `mty-codegen-wasm` emits core Wasm modules via `wasm-encoder
...
- Capability imports: `(import "mighty" "log" (func (param i32 i32)))`.
...
`mty-codegen-llvm` exists as a feature-gated placeholder. Default
...
- `sdust_runtime::extern_loader` — `libloading` registry; opens libc
  ...
  overrides via `mighty.toml [extern]` table (A53).
...
### `mty build` CLI subcommand

```
mty build [PATH] [--debug] [--release] [--target TARGET] [--out-dir DIR]
```
...
### `mty run` switches to JIT (A48)

`mty run <file>` now tries Cranelift JIT first. On
`CodegenError::Unsupported(_)` (the slice-8 backend covers a narrow
MtyIR subset), it falls back transparently to the slice-7 runtime path
...
| MT8007 | codegen rejected MtyIR shape |
...
All exercised by `crates/mty-driver/tests/conformance_codegen.rs`.
...
| A48 | `mty run` defaults to JIT with interpreter fallback |
...
| A53 | Extern resolution via `libloading` against host libc, overridable in `mighty.toml` |
...
| Example | `mty run` (JIT or fallback) | `mty build --target native` | `mty build --target wasm32-wasi` |
|---|:-:|:-:|:-:|
...
`mty run`. Full MtyIR coverage in the native backend (Result, ADT
...
- Full MtyIR coverage in native codegen (ADT/Result/?/agent dispatch)
...
- `crates/mty-codegen-cranelift/src/lib.rs` — backend entry points
- `crates/mty-codegen-cranelift/src/lower.rs` — MtyIR → cranelift IR
- `crates/mty-codegen-cranelift/src/jit.rs` — JIT driver
- `crates/mty-codegen-cranelift/src/object.rs` — AOT object writer + linker
- `crates/mty-codegen-cranelift/src/abi.rs` — call-conv + type lowering
- `crates/mty-codegen-cranelift/src/layout.rs` — ADT layout
- `crates/mty-codegen-cranelift/src/mono.rs` — monomorphization
- `crates/mty-codegen-cranelift/src/runtime_imports.rs` — runtime ABI sigs
- `crates/mty-codegen-wasm/src/emit.rs` — Wasm module emitter
- `crates/mty-codegen-wasm/src/target.rs` — `WasmTarget` enum
- `crates/mty-codegen-llvm/src/lib.rs` — LLVM scaffold
- `crates/mty-runtime/src/codegen_abi.rs` — runtime ABI bridge
- `crates/mty-runtime/src/arena.rs` — bumpalo arena stack
- `crates/mty-runtime/src/extern_loader.rs` — libloading registry
- `crates/mty-runtime/src/supervisor_orchestrator.rs` — restart engine
- `crates/mty-driver/src/build.rs` — `mty build` pipeline
- `crates/mty-cli/src/cmd/build.rs` — CLI subcommand
- `crates/mty-cli/src/cmd/run.rs` — JIT-first run path
...
- `docs/reference/cli/mty-build.md` — build CLI reference
- `docs/spec/v0.1-amendments.md` — A46..A53
- `tests/conformance/codegen/*` — 4 case dirs
- `crates/mty-driver/tests/conformance_codegen.rs` — 7-case driver

## End of v0.1

Slice 8 closes Mighty v0.1. The language is feature-complete per
spec §31. The pre-v0.1 roadmap (`README.md`'s slice table) is now
fully shipped. v0.2 work begins with the LLVM backend, full MtyIR
