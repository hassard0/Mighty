...
use crate::ids::*;
use crate::nodes::*;
use mty_ast::{AgentDecl, AstNode, ProtocolDecl, SupervisorDecl};
...
pub fn lower_agent(ctx: &mut LoweringCtx, a: AgentDecl) -> AgentId {
    ...
    let handlers: Vec<HirOnHandler> = a
        ...
        .map(|h| {
            ...
            let mut names = h.0.children().filter_map(mty_ast::Name::cast);
    ...
    let methods: Vec<FnId> =
        a.0.descendants()
            .filter_map(mty_ast::FnDecl::cast)
...
pub fn lower_protocol(ctx: &mut LoweringCtx, p: ProtocolDecl) -> ProtocolId {
    ...
    let messages: Vec<HirProtocolMsg> =
        p.0.descendants()
            .filter_map(mty_ast::ProtocolMsg::cast)
            .map(|m| {
                let msg_name =
                    m.0.children()
                        .find_map(mty_ast::Name::cast)
