#!/usr/bin/env bash
# demos/05_notetris_web/web/serve.sh — stage + serve the demo on
# http://localhost:8000 with zero deps beyond Python 3.
#
...
WASM="$ROOT/demos/05_notetris_web/target/main.wasm"
if [[ ! -f "$WASM" ]]; then
  echo "serve: $WASM not built yet — building now..."
  "$ROOT/target/debug/mty.exe" build --target wasm32-web \
    --out-dir "$ROOT/demos/05_notetris_web/target" \
    "$ROOT/demos/05_notetris_web/src/main.mty"
fi
cp "$WASM" "$STAGE/main.wasm"

echo "serving $STAGE on http://localhost:$PORT  (Ctrl-C to stop)"
cd "$STAGE"
# Prefer `python` (Windows / generic) over `python3` because on Windows
# `python3` is aliased to the MS Store installer launcher when no real
# python3 binary is on PATH.
