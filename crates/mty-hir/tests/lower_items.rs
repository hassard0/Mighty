...
fn lower_lambda_expr() {
    ...
    let has_lambda = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::Lambda { .. }));
...
fn lower_if_let_expr() {
    ...
    let has_iflet = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::IfLet { .. }));
...
fn lower_plain_if_still_works() {
    ...
    let has_if = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::If { .. }));
    let has_iflet = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::IfLet { .. }));
...
fn lower_run_expr() {
    ...
    let has_run = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::Run(_)));
...
fn lower_turbofish_path() {
    ...
    let has_pg = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::PathGeneric { .. }));
...
fn lower_plain_path_still_works() {
    ...
    let has_p = p
        .exprs
        .iter()
        .any(|(_, e)| matches!(e, mty_hir::HirExpr::Path(_)));
...
fn lowers_arena_short() {
    ...
    if let Some(tail_id) = block.tail {
        ...
        assert!(
            matches!(tail, mty_hir::HirExpr::Arena { .. }),
