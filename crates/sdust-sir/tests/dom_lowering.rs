...
//! Stardust source; the interpreter routes the same builtin through
//! `host.extern_call("dom.<op>", ...)` for headless test runs.

mod common;

use common::compile;
use sdust_sir::sir::{BuiltinId, FnRef, Rvalue, Stmt};
...
fn dom_ops_in(prog: &sdust_sir::sir::Program) -> Vec<String> {
    let mut out = Vec::new();
    for f in &prog.fns {
        for blk in &f.blocks {
...
fn dom_set_text_lowers_to_builtin_dom_op() {
    ...
    let dump = sdust_sir::dump::dump_program(&prog);
    assert!(
