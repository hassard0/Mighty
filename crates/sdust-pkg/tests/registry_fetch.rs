...
//! `cargo test -p sdust-pkg --test registry_fetch -- --ignored`.
...
fn fetches_real_release_index() {
    ...
    let result = sdust_pkg::fetch::registry::load_index_for(tmp.path(), slug, true);
    // This repo has no Stardust-shaped tags, so the index parses with
    ...
    // Cache file written.
    let cache = sdust_pkg::registry::cache_path(tmp.path(), slug);
    assert!(cache.exists());
}
