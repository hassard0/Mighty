...
use super::ctx::*;
use crate::ir::*;
use mty_hir::{HirLiteral, HirPat, PatId};
...
pub fn lower_pat_match(
    ...
    match pat {
        ...
        HirPat::Literal(lit) => {
            ...
            let cmp = fb.fresh_temp(IrTy::Bool);
            fb.push_stmt(Stmt::Assign(
                Place::local(cmp),
        ...
        HirPat::Enum { path, args } => {
            ...
            fb.set_term(Term::SwitchVariant {
                ...
                adt: mty_types::AdtId(0), // interpreter ignores the AdtId; uses Enum.variant
        ...
        HirPat::Range { lo, hi, inclusive } => {
            ...
            let cmp = fb.fresh_temp(IrTy::Bool);
            let cmp2 = fb.fresh_temp(IrTy::Bool);
            // discr >= lo
            fb.push_stmt(Stmt::Assign(
                Place::local(cmp),
                Rvalue::BinOp(
            ...
            let combined = fb.fresh_temp(IrTy::Bool);
            fb.push_stmt(Stmt::Assign(
...
fn bind_pat_recursive(ctx: &mut LowerCtx, fb: &mut FnBuilder, pat: &HirPat, rhs: Operand) {
    match pat {
        HirPat::Binding { name, sub } => {
            let l = fb.new_local(name.clone(), IrTy::Error, true, LocalSource::UserLet);
        ...
        HirPat::Tuple(parts) => {
            ...
            for (i, sp) in parts.iter().enumerate() {
                let elt = fb.fresh_temp(IrTy::Error);
                fb.push_stmt(Stmt::Assign(
                    Place::local(elt),
                    Rvalue::TupleRead {
        ...
        HirPat::Struct { fields, .. } => {
            ...
            for (idx, (_name, sub)) in fields.iter().enumerate() {
                if let Some(sp) = sub {
                    let f = fb.fresh_temp(IrTy::Error);
                    fb.push_stmt(Stmt::Assign(
                        Place::local(f),
                        Rvalue::FieldRead {
                            receiver: Place::local(temp),
...
fn local_ty(fb: &FnBuilder, p: &Place) -> IrTy {
...
fn parse_int_suffix(s: Option<&str>) -> mty_types::IntKind {
...
fn pat_to_const(p: &HirPat) -> Const {
    match p {
        ...
        _ => Const::Int(0, mty_types::IntKind::IntInfer),
    }
}
