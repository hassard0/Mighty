...
> and `?Msg(...)` call site; diagnostic code SD3011
...
`sendable::sendable_reason(...)`. A `Some(reason)` triggers a SD3011
...
field-level validation runs at the derive site itself; SD3011 catches
...
fn bad_ref(g: Greeter, x: I32) -> Unit {
  // SD3011: argument 1 (`&I32`) is not Sendable: contains a `&T`
  // reference (references never cross agent boundaries)
  g!Greet(&x)
}

fn bad_cap(s: SomeAgent, fs: Fs) -> Unit {
  // SD3011: argument 1 (`Fs`) is not Sendable: capability handle `Fs`
...
The two run independently — the type-checker phase emits SD3011, the
borrow checker emits SD3009 / SD3008 — and both must pass for a send
to be sound.
...
- v0.4: cross-package Sendable propagation: when an external crate
  marks a type Sendable, our SD3011 check should see the bound
