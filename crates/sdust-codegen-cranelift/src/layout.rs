...
pub fn primitive_layout(t: &SirTy) -> Option<Layout> {
    Some(match t {
        SirTy::Bool => Layout::scalar(1),
        SirTy::Char => Layout::scalar(4),
        SirTy::Unit | SirTy::Never => Layout::ZST,
        SirTy::Int(k) => match k {
        ...
        SirTy::Duration | SirTy::Size => Layout::scalar(8),
        // Str / String / Bytes share (ptr, len) shape.
        SirTy::Str | SirTy::String | SirTy::Bytes => Layout {
            size: PTR_BYTES * 2,
            align: PTR_BYTES,
        },
        SirTy::Ref { .. } | SirTy::RawPtr(_) | SirTy::Cap { .. } => Layout::scalar(PTR_BYTES),
        SirTy::Dyn(_) => Layout {
            size: PTR_BYTES * 2,
            align: PTR_BYTES,
        },
        SirTy::Fn { .. } => Layout::scalar(PTR_BYTES),
        SirTy::Module(_) | SirTy::Param(_) | SirTy::Error => Layout::ZST,
        // Aggregates need the catalog.
        SirTy::Tuple(_) | SirTy::Array { .. } | SirTy::Adt(_, _) => return None,
    })
}

/// Compute a layout including aggregate (tuple/array/ADT) types.
pub fn layout_with_adts(t: &SirTy, adts: &[AdtRef]) -> Layout {
    if let Some(l) = primitive_layout(t) {
        return l;
    }
    match t {
        SirTy::Tuple(elems) => layout_struct(elems.iter().map(|e| layout_with_adts(e, adts))),
        SirTy::Array { elem, len } => {
